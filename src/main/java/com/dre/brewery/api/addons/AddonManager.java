...
public final class AddonManager {
    ...
    public AddonManager(final BreweryPlugin plugin) {
        this.plugin = plugin;
    ...
    public final void unloadAddons() {
        if (LOADED_ADDONS.isEmpty()) {
            return;
        }

        Logging.log("Disabling " + LOADED_ADDONS.size() + " loaded addon(s)...");
        for (final var addon : LOADED_ADDONS) {
            this.unloadAddon(addon);
        }
    }

    public final void unloadAddon(final BreweryAddon addon) {
        if (addon == null) {
            Logging.warningLog("Tried to unload an addon that doesn't exist.");
            return;
        }
        final var n = addon.getAddonInfo() == null ? addon.getClass().getSimpleName() : addon.getAddonInfo().name();
        try {
        ...
        try {
            final var field = BreweryAddon.class.getDeclaredField("classLoader");
            field.setAccessible(true);
            final var classLoader = (URLClassLoader) field.get(addon);
            classLoader.close();
    ...
    public final void reloadAddons() {
        ...
        final var loaded = LOADED_ADDONS.size();
        if (loaded > 0) Logging.log("Reloaded " + loaded + " addon(s)");
    }

    public final ConcurrentLinkedQueue<BreweryAddon> getAddons() {
        return LOADED_ADDONS;
    }


    public final void loadAddons() {
        final var files = this.addonsFolder.listFiles((dir, name) -> name.endsWith(".jar")); // Get all files in the addons folder that end with .jar
        if (files == null) {
            return;
        }

        for (final var file : files) {
            this.loadAddon(file); // Go read the documentation below to understand what this does.
        }
     ...
     */
    public final void loadAddon(final File file) {
        try {
            ...
            final var mainClass = this.getMainClass(file, classLoader); // Get all our loaded classes.

            final BreweryAddon addon;
            try {
            ...
            try {
                ...
                final var infoField = BreweryAddon.class.getDeclaredField("addonInfo");
                ...
                final var classLoaderField = BreweryAddon.class.getDeclaredField("classLoader");
                classLoaderField.setAccessible(true);
                final var loggerField = BreweryAddon.class.getDeclaredField("logger");
                loggerField.setAccessible(true);
                final var fileManagerField = BreweryAddon.class.getDeclaredField("addonFileManager");
                fileManagerField.setAccessible(true);
                final var addonConfigManagerField = BreweryAddon.class.getDeclaredField("addonConfigManager");
                addonConfigManagerField.setAccessible(true);
                final var addonFile = BreweryAddon.class.getDeclaredField("addonFile");
     ...
     */
    private Class<? extends BreweryAddon> getMainClass(final File jarFile, final ClassLoader classLoader) {
        try (final var jarInputStream = new JarInputStream(new FileInputStream(jarFile))) {
            JarEntry jarEntry;
            while ((jarEntry = jarInputStream.getNextJarEntry()) != null) { // Just iterate through every file in the jar file and check if it's a compiled java class.
                if (jarEntry.getName().endsWith(".class")) {
                    ...
                    final var className = jarEntry.getName().replaceAll("/", ".").replace(".class", "");
                    try {
                        final Class<?> clazz;
                        try {
